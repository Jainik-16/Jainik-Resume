...
export default function CreateJobOpeningForm() {
  ...
  useEffect(() => {
    async function fetchOptions() {
      try {
        const [companies, departments, employment_types, designations, locations] = await Promise.all([
          axios.get(' http://172.23.88.43:8000/api/resource/Company?fields=["name"]', API_AUTH),
          axios.get(' http://172.23.88.43:8000/api/resource/Department?fields=["name"]', API_AUTH),
          axios.get(' http://172.23.88.43:8000/api/resource/Employment Type?fields=["name"]', API_AUTH),
          axios.get(' http://172.23.88.43:8000/api/resource/Designation?fields=["name"]', API_AUTH),
          axios.get(' http://172.23.88.43:8000/api/resource/Location?fields=["name"]', API_AUTH),
        ])
        setOptions({
          companies: companies.data.data.map((d: any) => d.name),
          departments: departments.data.data.map((d: any) => d.name),
