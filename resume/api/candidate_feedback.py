# # resume/api/candidate_feedback.py
...
#     FIXED VERSION - Uses direct SQL to avoid doctype issues
#     """
#     try:
#         # Fetch all Interview Feedback records using SQL
...
#                 # Try to fetch skill assessments - but don't fail if table doesn't exist
#                 try:
#                     # First check if we can find the child table
...
#                                 total_rating += skill_data["rating"]
                            
#                             if len(skill_assessments) > 0:
#                                 avg_rating = round(total_rating / len(skill_assessments), 1)
#                 except:
#                     # Skills table doesn't exist or error - continue without skills
#                     skill_assessments = []
#                     avg_rating = 0
